...
// .wrangler/tmp/bundle-kjCIos/strip-cf-connecting-ip-header.js
...
var TokenPayloadSchema = external_exports.object({
  ...
  userType: external_exports.enum(["owner", "admin"]),
  role: external_exports.enum(["admin", "reviewer", "director", "readonly"]).optional(),
...
var PropertySchema = external_exports.object({
  ...
  tenantId: external_exports.string().uuid(),
  ownerUserId: external_exports.string().uuid(),
  streetAddress: external_exports.string(),
  unit: external_exports.string().optional().nullable(),
  city: external_exports.string(),
  state: external_exports.string().length(2),
  zip: external_exports.string(),
  parcelId: external_exports.string().optional().nullable(),
  latitude: external_exports.string(),
  longitude: external_exports.string(),
  commissionDistrict: external_exports.string().optional().nullable(),
  inJurisdiction: external_exports.boolean(),
  createdAt: external_exports.date()
});
var CreatePropertySchema = external_exports.object({
  streetAddress: external_exports.string().min(1),
  unit: external_exports.string().optional(),
  city: external_exports.string().min(1),
  state: external_exports.string().length(2),
  zip: external_exports.string().regex(/^\d{5}$/),
  parcelId: external_exports.string().optional(),
  latitude: external_exports.string().regex(/^-?[0-9]+\.?[0-9]*$/),
  longitude: external_exports.string().regex(/^-?[0-9]+\.?[0-9]*$/),
  commissionDistrict: external_exports.string().optional(),
  inJurisdiction: external_exports.boolean().default(false)
});
var ApplicationStatusEnum = external_exports.enum([
  "draft",
  "submitted",
  "under_review",
  "info_requested",
...
var ApplicationSchema = external_exports.object({
  id: external_exports.string().uuid(),
  tenantId: external_exports.string().uuid(),
  propertyId: external_exports.string().uuid(),
  userId: external_exports.string().uuid(),
  localAgentId: external_exports.string().uuid().nullable(),
  applicationNumber: external_exports.string(),
  type: external_exports.enum(["new", "renewal"]),
  status: ApplicationStatusEnum,
  submittedAt: external_exports.date().nullable(),
  decidedAt: external_exports.date().nullable(),
  decidedBy: external_exports.string().uuid().nullable(),
  decisionNotes: external_exports.string().nullable(),
  inspectionDate: external_exports.date().nullable(),
  attestations: external_exports.record(external_exports.boolean()).nullable(),
  createdAt: external_exports.date(),
...
var CreateApplicationSchema = external_exports.object({
  propertyId: external_exports.string().uuid(),
  type: external_exports.enum(["new", "renewal"]),
  localAgentId: external_exports.string().uuid().optional(),
  attestations: external_exports.record(external_exports.boolean()).optional()
});
var SubmitApplicationSchema = external_exports.object({
  attestations: external_exports.record(external_exports.boolean())
});
var LicenseStatusEnum = external_exports.enum(["active", "expired", "suspended", "revoked"]);
var LicenseSchema = external_exports.object({
  id: external_exports.string().uuid(),
  tenantId: external_exports.string().uuid(),
  applicationId: external_exports.string().uuid(),
  propertyId: external_exports.string().uuid(),
  licenseNumber: external_exports.string(),
  issuedDate: external_exports.date(),
  expirationDate: external_exports.date(),
  status: LicenseStatusEnum,
  suspendedAt: external_exports.date().nullable(),
  suspendedReason: external_exports.string().nullable(),
  createdAt: external_exports.date()
});
var PaymentStatusEnum = external_exports.enum(["pending", "succeeded", "failed", "refunded"]);
var PaymentSchema = external_exports.object({
  ...
  feeType: external_exports.enum(["new_license", "renewal", "late_fee"]),
  status: PaymentStatusEnum,
  paidAt: external_exports.date().nullable(),
  createdAt: external_exports.date()
});
var CreatePaymentIntentSchema = external_exports.object({
  applicationId: external_exports.string().uuid(),
  amount: external_exports.number().positive(),
  feeType: external_exports.enum(["new_license", "renewal", "late_fee"])
});
var DocumentTypeEnum = external_exports.enum([
  "inspection_report",
  "proof_of_ownership",
  "insurance",
  "attestation",
...
var DocumentSchema = external_exports.object({
  ...
  virusScanStatus: external_exports.enum(["pending", "clean", "infected"]),
  uploadedAt: external_exports.date()
});
var RequestUploadUrlSchema = external_exports.object({
  filename: external_exports.string(),
  mimeType: external_exports.string(),
  documentType: DocumentTypeEnum
});
var ApiSuccessResponseSchema = external_exports.object({
  data: external_exports.unknown(),
  meta: external_exports.object({}).optional()
});
...
// src/routes/auth.ts
var SESSION_COOKIE_NAME = process.env.SESSION_COOKIE_NAME || "sentinel_str_session";
var SESSION_MAX_AGE_SECONDS = 60 * 60 * 24 * 7;
var DEFAULT_TENANT_ID = "11111111-1111-4111-8111-111111111111";
...
__name(ensureSeededUsers, "ensureSeededUsers");
function createSession(email) {
  const token = `${crypto.randomUUID()}${crypto.randomUUID()}`;
  const expiresAt = Date.now() + SESSION_MAX_AGE_SECONDS * 1e3;
...
authRouter.post("/register", async (c) => {
  ...
  const user = {
    ...
    phone: parsed.data.phone?.trim() || void 0,
...
// src/index.ts
var app = new Hono2();
var allowedOrigins = (process.env.CORS_ORIGINS || "http://localhost:3000,http://localhost:3003").split(",").map((origin) => origin.trim()).filter(Boolean);
app.use(
  "*",
  cors({
    origin: (origin) => {
...
// .wrangler/tmp/bundle-kjCIos/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-kjCIos/middleware-loader.entry.ts
